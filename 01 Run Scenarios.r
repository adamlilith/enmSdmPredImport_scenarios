...
###
### The purpose of this script is to 1) generate simulate data for each scenario in each "experiment"; 2) apply species distribution models to each simulated data set; and 3) evaluate the ability of the models to measure variable importance. The results for these experiments are presented in Smith & Santos.
###
### Note that this script is really not intended to be run on a single process all at once. Doing so will likely require a few years to complete on a modern desktop computer. Some of the simpler scenarios can be completed using a single process on a single computer in a few hours or a few days, but generally it is advisable to split jobs up by subsetting the set of parameters used to explore the parameters space, then training/testing models using the restricted set of parameters. For example, you could start an R process for the [sample size] experiment using the smallest sample size, start another R process using the next-larger size, and so on. Obviously the utility of this is limited to the number of cores and memory available on your computer. You can speed things up by using more than one computer, of course. This strategy will be greatly facilitated by using computers that share a common server to which they can read/write. If you use this strategy, the best gains will occur if the master working directory in which the scripts, simulated data, models, and evaluations are saved is all on the server.
###
### The [resolution], [bivariate], [extra false variable], and [missing true variable] experiments are so intensive that they are set up to take advantage of "pseudo" high-performance computing across computers linked only by a common server to which they have read/write access (pseudoHPC: see https://github.com/adamlilith/pseudoHPC). Here, the code sets up a common data frame across processes of jobs to do. The processes then check to see if a particular job has been started.  If not, they take the job and write a small "starts" file to the server so other processes know this job is reserved. When the job is complete the process writes another small "stops" file to the server, then checks the "starts" folder to see if there are remaining jobs to do. The user must make sure in the end that all jobs are completed. This may not occur if there is a runtime error or if access to the server is lost, for example. To ensure all jobs are complete simply delete the files in the "starts" folder and copy over the files in the respective "stops" folder. Then restart the experiment again.  The code will cycle through all jobs and determine if any are indeed complete.
...
#################
	...
	brtParams <- function(type) {
		...
		if (type %in% c('logistic', 'logisticShift')) {
		
			bestParams <- read.csv('./Results/tune brt & rf for logistic responses/Parameters of Best BRT Models.csv')
			...
			lr <<- c(0.005, 0.001, 0.0005, 0.0001) # learning rate
			tc <<- c(1, 2, 3) # tree complexity
			bf <<- 0.6 # bag fraction
			maxTrees <<- 8000 # maximum number of trees
			brtBg <<- 200 # background sites
...
# say('####################')
	...
	# progress <- progress[order(progress$sizeResampled, decreasing=TRUE), ]
	...
	# for (algo in algos) {
		...
		# while (length(started) < nrow(progress)) {
			...
			# sizeResampled <- progress$sizeResampled[doing]
			# noise <- progress$noise[doing]
			# geography <- list(T1=list(type='linear', min=-1, max=1, noise=noise), F1=list(type='random', min=-1, max=1))
			...
			# simExists <- file.exists(paste0(scenarioDir, '/', simDir, '/sizeResampled=', sizeResampled, ' noise=', round(noise, 2), ' sim ', prefix(max(iters), 4), '.RData'))
