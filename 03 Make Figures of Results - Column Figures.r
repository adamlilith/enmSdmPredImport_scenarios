...
say('###################################')
	...
	plotSimpleResp <- function(nudge, ylim, yTicks, ylab, lab, rand, trueField, controlField, falseField, controlLab, omniHasFalse, expectHigher) {
		...
		for (countAlgo in seq_along(algos)) {
			...
			if (!is.null(controlField)) rect(sdmControl, at=countAlgo, width=width, col='gray', border=borderOmniControl, lwd=0.8)
			rect(sdmTrue, at=countAlgo - nudge, width=width, col=colTrue, border=borderTrue, xpd=NA, lwd=0.8)
			rect(sdmFalse, at=countAlgo + nudge, width=width, col=colFalse, border=borderFalse, xpd=NA, lwd=0.8)
